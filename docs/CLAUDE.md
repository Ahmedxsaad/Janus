...
| 2026-08-06 | Claude (for Ghassen Naouar) | deploy/pypi-release.md covers the second distribution: janus-argos has its own pending publisher, its own `argos-v` tag namespace, and a Linux route that is a release attachment rather than a wheel. Its status line is corrected: janus-datahub is published, janus-argos is not (D-147) |
