...
| 2026-08-06 | Claude (for Ghassen Naouar) | plan/11-judge-readiness.md added: the delivery checklist from merged codebase to scored submission, ten phases (J-00 to J-09) sequencing the runbooks in deploy/ rather than restating them. It also carries the decision that plan/ is rewritten in place into judge-facing documents, which will change rules 1 and 3 here once J-03 runs (D-142, extended by D-143: the repository rename leads, the demo VM is reconfigured, site/ ships to Cloudflare, the README gets cut, and the video runs on a real project) |
