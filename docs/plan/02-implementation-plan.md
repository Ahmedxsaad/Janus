...
>
> **Convention:** ✅ = verified against DataHub docs/repo (July 2026). ⚠️ = exact symbol/behavior to
...
WRITE-BACK PRIMITIVES (all OSS): raiseIncident ✅ · structured properties ✅ · tags/terms/owners ✅
· knowledge documents (save_document) ✅ · guarding assertions as open-assertions YAML + entities ✅
```
...
```bash
# Python 3.10+ (Agent Context Kit requires 3.10+)               ✅
python3 -m pip install --upgrade pip wheel setuptools
python3 -m pip install --upgrade acryl-datahub                  # CLI + Python SDK   ✅
datahub version

# Start local DataHub (needs Docker + ~2 CPUs / 8GB free)       ✅
...
datahub datapack load showcase-ecommerce                        # ~1,050 cross-platform entities  ✅
...
# Reset if wedged
datahub docker quickstart --stop      #   or full wipe:  datahub docker nuke     ✅
```

**Project Python deps** (`requirements.txt`):
```
acryl-datahub[datahub-rest]        # SDK + REST emitter          ✅
datahub-agent-context              # Agent Context Kit toolset   ✅ (PyPI 1.5.x)
...
```
modelguard/
├── LICENSE                      # Apache 2.0 — set repo License in GitHub "About" (judging requirement) ✅
...
├── examples/                    # SAMPLE OUTPUTS for judges (no run needed) — recommended by rules ✅
...
Verified SDK surface (from DataHub AI/ML tutorial ✅):

```python
from datahub.sdk import DataHubClient
from datahub.sdk.ml_entities import MLModelGroup, MLModel          # ⚠️ confirm exact module path
...
# 2) Training run (DataProcessInstance, MLFLOW_TRAINING_RUN subtype)  ✅
...
model.add_training_job(DataProcessInstanceUrn("credit_risk_v3_run"))  # ✅
client._emit_mcps(model.as_mcps())

# 3) Wire training run to REAL datapack tables (this is what makes lineage meaningful) ✅
...
- `mlfeature_create.py` — `MLFeature(... sources=[<dataset/column urns>])`  ⚠️ copy verbatim
...
> **Column-level lineage is Dataset→Dataset only** ✅. Feature→dataset lineage is expressed via the ML
...
have something to traverse (verified SDK ✅):
...
```python
# INCIDENT (OSS-native, via GraphQL; Python SDK "coming soon") ✅
...
```
```bash
# STRUCTURED PROPERTY definition (CLI) ✅ — then assign via GraphQL/OpenAPI (§6.2)
...
def models_at_risk(client, failing_table_urn):
    ...
    dl = client.lineage.get_lineage(                       # ✅ traverse downstream
        source_urn=failing_table_urn, direction="downstream", max_hops=5,
        filter=F.entity_type("mlModel"),                   # ⚠️ confirm ML entity_type filter names
    )
...
- **Incident** on each at-risk model/deployment — `raiseIncident` (`type` ∈ `OPERATIONAL, FRESHNESS,
  VOLUME, COLUMN, SQL, DATA_SCHEMA, CUSTOM` ✅), title/description generated by the LLM from the traversal.
- **Tag** `model-at-risk` + **glossary term** on the model (MCP `add_tags`/`add_terms` ✅, or SDK).
...
- **Model Impact Report** → knowledge **document** (`save_document` ✅) linked to the model.
...
def leakage_findings(client, model_urn, label_source_column_urn):
    ...
    for feat in features:
        ...
        for col in feature_source_columns(client, feat):         # via ML `sources` aspect
            cone = client.lineage.get_lineage(                    # ✅ column-level, upstream
...
- Read the training run's **input dataset schema as-of the run timestamp** vs the source's **current**
  `schemaMetadata`, using the **Timeline / Schema-History API** ✅:
  ```bash
...
### 6.1 Incidents ✅ (`graph.execute_graphql`)
```graphql
...
```
(or OpenAPI v3 `POST /openapi/v3/entity/mlModel/<url-encoded-urn>/structuredProperties`.) ✅
...
`update_description`) — need `TOOLS_IS_MUTATION_ENABLED=true` ✅. From deterministic scripts, use the SDK
PATCH builders instead of the agent so writes are reproducible in tests.

### 6.4 Knowledge documents ✅
...
(`add_tags`, `update_description`, `add_glossary_terms`, `set_domains`, `add_owners`, `save_document`) ✅.
Incidents/assertions are **not** in the toolset → add them as **custom LangChain tools** wrapping our
`writeback/` functions.

⚠️ **Verify exact import symbols** (`pip show datahub-agent-context` then introspect — the public blog's
snippet was OCR-garbled). Approximate shape:
```python
from datahub.sdk import DataHubClient
from datahub_agent_context.langchain_tools import build_langchain_tools   # ⚠️ confirm name
...
- `modelguard watch` — event-driven via the **DataHub Actions framework** (Kafka `EntityChangeEvent`),
  with a **polling fallback** so the demo never depends on Kafka timing. ⚠️ Actions setup adds complexity —
...
### 8.1 `datahub-ml-guard` Skill (primary — first ML skill in the registry) ✅ gap confirmed
Repo layout mirrors existing skills (`skills/<name>/SKILL.md`, `references/`, `templates/`; standards are
symlinked; shared CLI/MCP signatures live in `shared-references/`) ✅. Anthropic Skill format = YAML
...
### 8.2 MCP mutation tool `raise_incident` (stretch) ✅ gap confirmed
...
- [ ] Public repo, **Apache-2.0 license file**, License shown in GitHub **About** ✅ (hard requirement)
- [ ] Project URL + testing instructions (local `quickstart.sh`; credentials if any)
- [ ] `examples/` folder: impact report, guarding-assertion YAML, incident payload, the skill ✅
...
| Exact SDK/agent import symbols differ | Med | ⚠️-flagged everywhere; `pip show` + introspect; pin versions in `requirements.txt`. |
...
```python
# --- client ---
DataHubClient.from_env()                                             # env: DATAHUB_GMS_URL/_TOKEN     ✅
DataHubGraph(DatahubClientConfig(server=..., token=...))            # for execute_graphql             ✅
...
client.lineage.get_lineage(source_urn=, direction="upstream|downstream",
    max_hops=, source_column=, filter=FilterDsl...)                 # ✅
client.lineage.infer_lineage_from_sql(query_text=, platform=,
    default_db=, default_schema=)                                   # ✅

# --- ML entities ---
MLModelGroup(id=, platform=, name=, description=); .as_mcps()        # ✅
MLModel(id=, platform=, name=, version=, aliases=, hyper_params=, training_metrics=)
    .add_group(urn); .add_training_job(DataProcessInstanceUrn(run_id))   # ✅
client.create_training_run(run_id=, training_run_properties=MLTrainingRunPropertiesClass(...))  # ✅
client.add_input_datasets_to_run(run_id=, datasets=[...])           # ✅
client.add_output_datasets_to_run(run_id=, datasets=[...])          # ✅
# MLFeature(sources=[...]) / MLFeatureTable(mlFeatures=,mlPrimaryKeys=) / MLModelDeployment
#   → copy mlfeature_create.py, mlfeature_table_create.py, mlfeature_add_to_mlmodel.py  ⚠️

# --- write-back ---
graph.execute_graphql(RAISE_INCIDENT | UPDATE_INCIDENT_STATUS | UPSERT_STRUCTURED_PROPERTIES)  # ✅
# CLI:  datahub properties upsert -f props.yaml                     # define structured props     ✅
# CLI:  datahub datapack load {showcase-ecommerce|nyc-taxi|healthcare}                           # ✅
# CLI:  datahub timeline --urn "<urn>" --category TECHNICAL_SCHEMA  # schema history for P3       ✅
# MCP write tools (TOOLS_IS_MUTATION_ENABLED=true): add_tags, add_terms, add_owners,
#   set_domains, add_structured_properties, update_description, save_document                     ✅
```

**Incident types:** `OPERATIONAL, FRESHNESS, VOLUME, COLUMN, SQL, DATA_SCHEMA, CUSTOM` ✅
