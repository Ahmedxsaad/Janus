...
| 2026-08-02 | Claude (for Ghassen Naouar) | Status after the first pass. Landed: F1, F2, F9, F12, F13 (D-085 to D-089), then F3, F10, F11 and the code half of F8 and F6 (D-090 to D-094). Three things this document asked for are written but **not measured**, because they need a live Quickstart nobody has run yet: the three new integration modules, the regenerated RESULTS.md carrying the P5/P6 rows, and F6's step 3 (scoring against a graph DataHub's own ingestion built). F3's third layer (a compare-and-set write) stays open on purpose: the SDK exposes a structured-property patch builder, but no live server has confirmed it, so the read-merge-write ceiling is documented instead of quietly swapped. New finding worth adding: `--infer` cannot resolve a label that lives in its own table, which is the common warehouse shape and the seeded graph's own (D-093) |
