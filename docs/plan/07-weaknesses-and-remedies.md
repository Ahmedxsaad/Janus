...
answer, and the most work.~~ **Closed by D-115 (T-14).**
...
| 2026-08-02 | Claude (for Ghassen Naouar) | Status after the first pass. Landed: F1, F2, F9, F12, F13 (D-085 to D-089), then F3, F10, F11 and the code half of F8 and F6 (D-090 to D-094). F3's third layer (a compare-and-set write) stays open on purpose: the SDK exposes a structured-property patch builder, but no live server has confirmed it, so the read-merge-write ceiling is documented instead of quietly swapped. Two things this document asked for are written but **not measured**: the regenerated RESULTS.md carrying the P5/P6 rows and the new falsifiability columns, and F6's step 3 (scoring against a graph DataHub's own ingestion built). The `--infer` label gap found while writing F8's tests was fixed rather than filed (D-095) |
| 2026-08-02 | Claude (for Ghassen Naouar) | F8's integration suite has now been run against a live Quickstart: 52 tests pass, and the first run found a defect the entire offline suite had passed through (a sensitive-source scan raising `NotImplementedError` while writing its own impact report, after the incident had already been written: D-096). That is this finding's argument demonstrated rather than asserted, and it is the strongest evidence in this document that `FakeGraph` coverage is not coverage |
