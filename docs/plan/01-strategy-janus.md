# Janus - Winning Strategy (Verified)
...
source columns). Janus turns that unified graph into the missing CI - the "from documenting the
...
| Criterion (equal weight) | What scores | How Janus maxes it |
|---|---|---|
...
| **2. Technical Execution** | Works **end-to-end**, robust; code does what the claim says. | Deterministic detection (graph analysis + schema diff); LLM only for reasoning/narrative + human-gated writes. Runs on **local Quickstart + seeded ML graph + planted-issue datapack** → judges rerun, same result. **Measured, not asserted:** Janus-Bench reports precision/recall/MTTD and **beats no-lineage baselines** (Great Expectations, Evidently). Idempotent write-back; batch **and** event-driven. One-command `quickstart.sh`. (§5) |
...
thinnest fields. Janus sits in **category 3** with Grand-Prize-quality build → strong EV on both the
...
Most entries stop at "it ran once in the video." Janus is engineered - and *measured* - like a system a
...
- **Measured, not asserted - "Janus-Bench."** Detectors are scored against ground truth built from
...
- **Secure by design - it writes to a governance graph.** Deterministic detection is prompt-injection-
  ...
  (OWASP **LLM06/LLM05**). Quotable privacy property: **Janus reads metadata + profiles, never raw
