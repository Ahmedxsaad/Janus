...
auditing a catalog types a command. But the one place ModelGuard has to be called
...
:func:`~modelguard.agent.pipeline.run_scan` and ``link_model`` calls
:func:`~modelguard.writeback.link.link_model`, which are the same functions
``modelguard scan`` and ``modelguard link`` call, so a finding found here is
found identically at the command line (modelguard/CLAUDE.md rule 2: four
triggers, one core).

Not a place where configuration is invented either. Both connect through
:func:`~modelguard.client.connect` and read :class:`~modelguard.config.ScanConfig`
...
from modelguard.client import DataHubConnection, connect
from modelguard.config import ScanConfig
from modelguard.writeback.link import LinkError, LinkResult
from modelguard.writeback.link import link_model as _link_model


def _resolve(conn: DataHubConnection, name: str, *, model: bool) -> str:
    """Turn a name or URN into exactly one URN, the way the CLI does.

    Imported lazily from the CLI module so importing :mod:`modelguard` does not
    import Typer and rich for a caller that only wants two functions.
    """
    from modelguard.cli import resolve_model, resolve_table
...
def scan_model(
    *,
    ...
    The same core ``modelguard scan`` runs. No LLM is used: prose is what a
    command-line report wants and a caller holding a
    :class:`~modelguard.agent.pipeline.ScanReport` has the findings themselves,
