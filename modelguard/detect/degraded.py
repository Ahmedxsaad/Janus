...
:mod:`modelguard.detect.coverage` correctly says why. That is honest, and on a
stranger's catalog it is also the whole answer: nothing was checked, come back
after somebody runs ``modelguard link``.

This module is the weaker answer offered in the meantime. Without column-level
lineage ModelGuard can still read three facts about the *tables* a model trains
...
:mod:`modelguard.detect.governance` already reads), and the datasets the catalog
