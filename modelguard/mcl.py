"""Reading DataHub's own change log, so ModelGuard can react instead of ask.

``modelguard watch`` polls. That is deliberate and it stays the default: polling
...
:mod:`modelguard.reconcile` and to ``watch``, for the same reason detection lives
...
default on purpose. Two ModelGuard deployments sharing one group id would split
...
ENV_KAFKA_BOOTSTRAP = "MODELGUARD_KAFKA_BOOTSTRAP"
ENV_SCHEMA_REGISTRY_URL = "MODELGUARD_SCHEMA_REGISTRY_URL"
ENV_KAFKA_GROUP_ID = "MODELGUARD_KAFKA_GROUP_ID"
...
class ChangeLog:
    ...
    Opened by ``modelguard watch --events`` and closed when it stops. The
    ...
    def __enter__(self) -> ChangeLog:
        ...
        try:
            ...
            raise ConfigError(
                ...
                "Install it with: pip install 'modelguard-datahub[kafka]'"
            ) from exc
