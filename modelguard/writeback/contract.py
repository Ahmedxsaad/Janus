...
ODCS can enforce it, not just ModelGuard.

Pure renderer
-------------
Like :func:`~modelguard.writeback.assertions.render_assertion_yaml` and
:func:`~modelguard.writeback.documents.render_impact_report`, this reads the graph
...
``physicalType``. The freshness expectation is the SLA ModelGuard would guard
...
volume or distribution "expectation" here, because ModelGuard does not measure one
...
class ContractError(Exception):
    """The model has no input table whose schema ModelGuard can contract.
...
def render_input_contract(
    ...
    property, so the contract declares exactly the checks ModelGuard would guard.
    ...
    # One freshness SLA entry per input table, all sharing the SLA ModelGuard
    # guards. `element` names the field a warehouse-side evaluator would read;
    # ModelGuard itself measures freshness from the operation aspect, but the
    ...
    sla_properties = [
        {
            ...
            "driver": "operational",
            "description": "ModelGuard freshness SLA; matches the emitted guarding assertion.",
        }
    ...
    contract = {
        ...
        "kind": "DataContract",
        "id": f"modelguard.input-contract.{model_name}",
        "name": f"Input data contract for {display_name}",
        "version": "1.0.0",
        "status": "active",
        "description": {
            "purpose": (
                f"Input requirements ModelGuard derived for the model {display_name}: "
            ...
            "usage": f"Generated by ModelGuard for {model_urn}.",
        },
