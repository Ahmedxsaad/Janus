"""ModelGuard's own scans, emitted into the graph as process runs.

Every other module in this package writes *about* somebody else's data. This one
writes about ModelGuard: each scan becomes a ``dataProcessInstance`` under a
...
- An incident stamped ``Raised by ModelGuard run scan-abc123`` currently sends a
...
- ModelGuard's own freshness becomes checkable by ModelGuard, using the same
...
#: The platform every ModelGuard flow, job and run hangs off. It names this
...
ORCHESTRATOR = "modelguard"

#: The one flow. ModelGuard has a single pipeline, so the flow id is the
...
FLOW_NAME = "ModelGuard"
FLOW_DESCRIPTION = (
    "The ModelGuard reliability agent. It reads column-level and ML lineage to find "
...
#: loop in `modelguard watch`), and claiming a schedule the process does not keep
#: would be a fact about ModelGuard that ModelGuard made up.
RUN_TYPE = DataProcessTypeClass.BATCH_AD_HOC


def _flow() -> DataFlow:
    """The single dataFlow every ModelGuard run belongs to."""
    return DataFlow(
...
def _emit_template(conn: DataHubConnection, mcps: Iterable[MetadataChangeProposalWrapper]) -> None:
    ...
    tag or an owner somebody had put on ModelGuard's own flow, every few seconds
    under ``modelguard watch``. This module sets neither, so it emits neither.
    """
    for mcp in mcps:
    ...
    conn.graph.emit_mcp(
        _run_event(
            ...
            result=DataProcessInstanceRunResultClass(
                ...
                # the helper would default to: the result is ModelGuard's own
