"""ModelGuard as an MCP server: the detectors, asked conversationally.

``modelguard scan`` and ``modelguard gate`` are triggers for a human at a terminal
...
ModelGuard's own narrator, it is whatever model the MCP client is running,
...
command, which is the same boundary ``modelguard gate`` draws for CI (D-052).
...
Run it
------
``modelguard-mcp`` starts the server on stdio, the transport Claude Desktop and
...
from modelguard.gate import GatePolicy, evaluate, summary
...
def check_gate(
    ...
    """Ask whether a model or table would pass ModelGuard's CI gate.

    The same policy ``modelguard gate`` enforces in a pipeline, asked
...
def main() -> None:
    """Entry point for the ``modelguard-mcp`` command. Serves over stdio.
