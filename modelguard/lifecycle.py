"""How long ModelGuard's findings stay open, read back out of the graph (T-16).

ModelGuard raises incidents (D-013) and resolves them when the problem goes away
...
---------------------------
Nowhere new. ``incidentInfo.created.time`` is when ModelGuard raised the
...
---------------------------
Only ModelGuard's. Every incident this project raises carries the run footer
...
#: prefix registered here. Keyed by :class:`~modelguard.models.FindingType` for
...
#: presence is also what marks an incident as ModelGuard's own: a human writing a
#: description by hand does not end it with this sentence, and no other tool
#: writes ModelGuard's run ids.
_RUN_FOOTER = re.compile(r"Raised by ModelGuard run (\S+?)\.\s*$")
...
class IncidentLifecycle:
    """One ModelGuard incident, and what the graph records about its lifetime."""
...
@dataclass(frozen=True)
class TypeLifecycle:
    """Every ModelGuard incident of one finding type, rolled up."""
...
def _lifecycle(
    ...
    """Read one incident, or None when it is not a classifiable ModelGuard one."""
...
def read_lifecycles(
    ...
    """Return every ModelGuard incident reachable from these models' resources.
