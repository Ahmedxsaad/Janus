...
tailing `modelguard watch` wants to know a lineage walk started too.
...
aspect content (modelguard/logs.py), so the bubble's sentence comes from the
...
from modelguard.logs import LOG_FIELDS, PHASE_FIELD
    ...
    def __init__(self, sink: Callable[[Event], None]) -> None:
        ...
        Args:
            sink: Called with one event per phase record. Must be safe to call
                ...
                :class:`~modelguard.argos.window.ArgosWindow` it is: a write to
