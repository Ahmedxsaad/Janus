...
catalogue: `modelguard watch` (one target, ModelGuard's own detectors) and
`modelguard companion` (owned assets, the general DataHub view) are the two in
...
from modelguard.argos.window import ArgosWindow, install_hint
...
class ArgosProducer:
    ...
    def __init__(
        ...
        *,
        ...
        logging.getLogger("modelguard").addHandler(self._handler)
    ...
    def _open(self, entity: str | None) -> None:
        ...
        if not entity:
            return
        from modelguard.argos.events import entity_link

        url = entity_link(entity)
        if url is None:
            self.send(Event(state="patrolling", title="set MODELGUARD_DATAHUB_UI_URL to open"))
            return
        webbrowser.open(url)
    ...
    def close(self) -> None:
        ...
        logging.getLogger("modelguard").removeHandler(self._handler)
        self._surface.close()
