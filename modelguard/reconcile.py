...
drops the ``mlFeatures`` that ``modelguard link`` attached. From that moment the
...
--------------------------------
A CronJob after the nightly ingest is what the ``modelguard-watch`` chart already
...
from modelguard.logs import logfmt
    ...
    *,
    ...
    if previous is None:
        ...
        return Relink(
            ...
            relinked=False,
            reason="no modelguard link is recorded on this model, so there is nothing "
            "confirmed to replay",
        )
