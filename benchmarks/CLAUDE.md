...
| 2026-08-05 | Claude (for Ghassen Naouar) | RESULTS.md gains an incident-lifecycle section (D-127, T-16), and its caveat is the section: on a benchmark graph a trial plants a failure and the next trial reverts it, so nearly every duration is the seconds between a plant and a restore. It is published with what it is not, above the table, because a raise-and-resolve loop closing per detector is the real measurement and an MTTR for a team is not |
