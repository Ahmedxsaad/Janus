...
4. Prefer a service failing fast and retrying (`Restart=always` with a real
   ...
   right. `janus watch` already fails loudly and specifically when
   DataHub is not yet reachable (janus/client.py); leaning on that
...
| Date | Author | Change |
|---|---|---|
| 2026-07-23 | Claude (for Ahmed Saad) | Initial version: azure/cloud-init.yaml, azure/janus-watch.service (D-057) |
...
| 2026-07-29 | Claude (for Ahmed Saad) | Custom domain + HTTPS via Caddy verified live (`https://janus.ahmedxsaad.me`); fixed the frontend password-change instructions to the real user.props mechanism; azure/Caddyfile.template added (D-064) |
...
| 2026-07-30 | Claude (for Ahmed Saad) | Redeployed the live VM onto D-067's fixes (it had cloned before that PR merged); found janus-watch.service's ExecStop does not actually stop docker compose run containers, a minor restart race left as a known gap (D-068) |
...
| 2026-08-01 | Claude (for Ghassen Naouar) | The live VM runs D-074's branch (fetch, `docker compose build janus-watch`, restart: compose builds `janus:local` from the repo, so a checkout alone changes nothing). Provisioning's clone token was still in the VM's `.git/config` and still valid, past the point D-062 says to revoke it; removed from the box, revocation on GitHub is on its owner (D-075) |
...
| 2026-08-06 | Claude (for Ahmed Saad) | D-136's rename touched dated "Verified live" claims it should not have: azure-vm.md and decision-log.md's D-064 were rewritten to name a domain (janus.ahmedxsaad.me) that did not exist on the date they record. Restored to what was actually verified that day (D-142) |
| 2026-08-06 | Claude (for Ahmed Saad) | The live VM (started by the user) migrated onto Janus over SSH: repo moved to /opt/janus/DataHub, janus:local rebuilt, modelguard-watch.service replaced with the tracked janus-watch.service and confirmed writing idempotent findings, Caddy repointed at janus.ahmedxsaad.me (the user's Cloudflare A record already in place) and confirmed serving real content over a fresh Let's Encrypt cert. Swap doubled to 8G, live and in cloud-init.yaml, precautionary ahead of the judging window rather than a fix for a repeat of D-071 (D-143) |
