...
Janus is bring-your-own-LLM. The provider, the model, and the key are three
configuration values, read together, with **no defaults**:

* ``JANUS_LLM_PROVIDER``: one of :data:`SUPPORTED_PROVIDERS`.
* ``JANUS_LLM_MODEL``: the provider's model id, verbatim.
* ``JANUS_LLM_API_KEY``: the credential for that provider.
...
ENV_LLM_PROVIDER = "JANUS_LLM_PROVIDER"
ENV_LLM_MODEL = "JANUS_LLM_MODEL"
ENV_LLM_API_KEY = "JANUS_LLM_API_KEY"
