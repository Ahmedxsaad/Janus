"""How a long-running Janus process talks to a log pipeline.

``janus watch`` is the one entry point that runs unattended for days, which
...
ENV_LOG_FORMAT = "JANUS_LOG_FORMAT"
...
#: (janus/argos/handler.py); a person reads it in the terminal. Neither
...
def configure_logging(*, level: int = logging.INFO, stream: TextIO | None = None) -> str:
    ...
    embedding Janus as a library owns its own logging, and a package that
    ...
    Raises:
        ConfigError: ``JANUS_LOG_FORMAT`` is set to an unknown value.
    """
