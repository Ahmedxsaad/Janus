...
:mod:`janus.env`, which is the only module that loads the git-ignored
...
#: `janus-seed`) is correct here and wrong anywhere else: told to somebody
...
def gms_token() -> str | None:
    ...
    it for exactly one purpose: passing it to :func:`janus.env.scrub` so a
