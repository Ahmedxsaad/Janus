...
Janus from a linter into something that leaves institutional memory behind.
...
def _document_id(model_urn: str, finding_type: FindingType, resource_urn: str) -> str:
    ...
    return f"janus-impact-{model_name}-{resource_hash}"
...
Freshness here is derived from metadata DataHub already holds. Janus did not \
query the warehouse. Scheduled evaluation of assertions and anomaly detection are \
DataHub Cloud features; the check logic above is Janus's own.
"""
...
Every edge above is a declared column-level lineage edge in DataHub. Janus \
...
Janus proves the derivation from lineage, not from the data. If the lineage \
...
(`{finding.training_run_urn}`). Janus compared it against the dataset's \
...
Janus compares the training-time schema snapshot against the current schema, \
...
Janus reports the derivation and the classification as DataHub holds them. It \
...
Janus reads the `deprecation` aspect and the model's recorded inputs. It has no \
...
the link (`janus link`) replaces it with a finding that can.
"""
...
def render_trust_waterfall(score: TrustScore) -> str:
    ...
    of the object (see :class:`~janus.models.TrustScore`). Empty for a model
...
def publish_impact_report(
    ...
    *,
    ...
    title = f"Janus impact report: {report_subject(finding)}"
