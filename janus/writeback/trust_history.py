...
``janus.trust_score`` is a single structured property, overwritten by every
...
``janus.trust_history``, oldest first, and the list is capped. This is a
recent trend a human reads on the model in the DataHub UI, not an audit log:
every entry also exists as the ``janus.run_id`` stamped on that run's
writes, which is where a full audit trail lives.

Why a structured property and not a timeseries aspect
-----------------------------------------------------
A timeseries aspect is what this would be in a world where Janus could add
