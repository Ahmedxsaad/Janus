...
What a card is not
------------------
Not an approval, and not a statement about the feature's values. Janus has
...
from janus.detect.column_marks import ColumnMarkIndex, derivation_chains, marked_ancestor
from janus.detect.governance import protected_attribute_index, sensitive_index
from janus.detect.leakage import feature_source_column
from janus.detect.schema_drift import training_snapshot
from janus.models import Finding, FreshnessSignal
from janus.writeback.documents import RUN_ID_PROPERTY, urn_hash
...
def _exposures(
    ...
    kinds: list[tuple[str, ColumnMarkIndex, str]] = [
        (
            ...
            "JANUS_SENSITIVE_TERM_URNS or JANUS_SENSITIVE_TAG_URNS",
        ),
        (
            "protected attribute",
            protected_attribute_index(conn, config),
            "JANUS_PROTECTED_ATTRIBUTE_TERM_URNS or JANUS_PROTECTED_ATTRIBUTE_TAG_URNS",
        ),
    ]
    ...
    if source is None:
        return FeatureFacts(
            ...
            unevaluated=(
                ...
                "(`janus.source_column`), so there is no column to walk back "
                "from. `janus link` is what writes it.",
            ),
        )
...
def render_feature_card(facts: FeatureFacts) -> str:
    ...
    lines = [
        ...
        "",
        f"A Data Card for one feature of `{facts.model_name}`, generated by Janus "
    ...
    lines += [
        ...
        "## Limits of this card",
        "",
        "- Janus has never read this feature's values. Nothing here is a "
...
def publish_feature_card(
    ...
    *,
    ...
    document = Document.create_document(
        id=f"janus-feature-card-{facts.feature_name}-{urn_hash(facts.feature_urn)}",
