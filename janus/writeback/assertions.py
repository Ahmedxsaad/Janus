...
the next time is the point. Janus writes a freshness assertion onto the
...
* **the assertion entity** in the graph, so it appears on the dataset's Quality
  tab, plus a run event recording the result Janus actually measured.
...
so Janus supplies the check logic itself and writes both.

The run event is not synthetic
------------------------------
``AssertionResult`` carries the freshness lag Janus computed from the
...
def assertion_spec(table_urn: str, sla_hours: float, freshness_field: str) -> dict[str, Any]:
    ...
    read. Janus evaluates the check from DataHub's operation aspect instead;
        ...
        "assertions": [
            {
                "entity": table_urn,
                "type": "freshness",
                "id_raw": f"janus.freshness.{table_name}",
                "description": (
                    f"Janus guarding assertion: {table_name} must change at least "
...
def upsert_guarding_assertion(
    ...
    *,
    ...
    info.source = AssertionSourceClass(
        # INFERRED: Janus derived this check from an observed failure, rather
