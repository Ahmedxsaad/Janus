...
Janus both reads terms (a label declaration, see detect/leakage.py) and
...
:func:`janus.writeback.labels.add_tag` does. Attaching a term that is already
...
* directly on a ``schemaField``, which is how Janus declares a label, and
...
declaration counts exactly as much as Janus's own (detect/leakage.py).
"""
...
def remove_term(conn: DataHubConnection, entity_urn: str, term_urn: str) -> bool:
    """Remove one Janus-owned term while preserving every other term."""
    existing = conn.graph.get_aspect(entity_urn, GlossaryTermsClass)
    if existing is None:
        return False
