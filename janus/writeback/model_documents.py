...
from janus.models import Finding, ModelRef, TrustScore
...
def render_model_card(facts: ModelFacts) -> str:
    ...
    lines = [
        ...
        "",
        "Generated by Janus from metadata in DataHub. Every fact below is read "
    ...
    if facts.training_metrics:
        lines += [
            ...
            "",
            "As recorded on the model by whatever trained it. Janus did not "
    ...
    lines += [
        ...
        *_gaps_section(facts),
        ...
        "- Janus has never read this model's training data or its predictions. "
...
def render_evidence_pack(facts: ModelFacts) -> str:
    ...
    unestablished: list[str] = [
        "- **Freshness of each input at training time.** Janus measures "
        ...
        "- **Anything about the data's contents.** Janus reads lineage and "
...
def publish_model_card(
    ...
    *,
    ...
    return _publish(
        ...
        facts,
        run_id=run_id,
        document_id=f"janus-model-card-{facts.model.name}-{urn_hash(facts.model.urn)}",
...
def publish_evidence_pack(
    ...
    *,
    ...
    return _publish(
        ...
        facts,
        run_id=run_id,
        document_id=f"janus-evidence-pack-{facts.model.name}-{urn_hash(facts.model.urn)}",
