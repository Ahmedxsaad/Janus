...
is carried by structured properties (``janus.trust_score``,
``janus.risk_flags``), which mlModel does accept. That split is arguably the
...
def raise_incident(
    ...
    *,
    ...
    Args:
        ...
        run_id: Identifier of the current Janus run, for the audit trail.
    ...
    if entity_type not in INCIDENT_ENTITY_TYPES:
        raise ValueError(
            ...
            "model-level risk with the janus.trust_score structured property."
        )
...
def resolve_incident(conn: DataHubConnection, incident_urn: str, message: str) -> None:
    ...
    Raises:
        IncidentWriteError: DataHub rejected the state change. Raised rather than
            ...
            DataHub's own incident ACTIVE while Janus reports the model recovered.
    """
    response = conn.graph.execute_graphql(
        _UPDATE_INCIDENT_STATUS,
        variables={
