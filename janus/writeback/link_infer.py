...
``janus link`` is the join no ingestion source writes, and every detector
...
it is the reason ``janus inventory`` says "not checked" forever.
...
which a human confirms and :func:`~janus.writeback.link.link_model` then
...
* **The label column.** First from a column that already carries the configured
  ...
  :attr:`~janus.config.ScanConfig.label_column_names`, matched
...
from janus.adapters import DeclaredLink, excluded_columns, missing_columns
from janus.client import DataHubConnection
from janus.config import ScanConfig
from janus.detect.column_marks import ColumnMarkIndex, marked_ancestor
...
#: ``janus_features`` leads because it is the key this project's own README
...
class InferenceError(LinkError):
    ...
    A subclass of :class:`~janus.writeback.link.LinkError` so a caller that
...
class LinkProposal:
    ...
    def command(self) -> str:
        ...
        parts = ["janus link", f"--model {shlex.quote(model)}", f"--features {table}"]
    ...
    for field in schema.fields:
        if field.fieldPath.lower() in wanted:
            return str(SchemaFieldUrn(dataset_urn, field.fieldPath)), (
                ...
                "(JANUS_LABEL_COLUMN_NAMES). This one is a guess: check it"
            )
