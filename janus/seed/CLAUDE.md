...
3. Column-level lineage is Dataset to Dataset only. Feature-to-dataset lineage
   ...
   under janus.source_column; detectors start their traversal there.
...
5. scenarios.py plants failures for the demo and benchmark; keep each scenario
   ...
   stamps janus.scenario into the aspect's customProperties so a reader can
...
| 2026-08-04 | Claude (for Ahmed Saad) | scenarios adds plant/revert_common_ancestor_label and plant/revert_label_lookalike, T-09's two confusable negatives. Both build their column-lineage mapping from a new `_LEAK_FREE_COLUMN_LINEAGE` baseline rather than spreading `spec.COLUMN_LINEAGE` directly: `_set_column_lineage` replaces the whole mapping, so spreading the raw seeded dict silently reintroduces the flagship leak's own edge alongside each scenario's shape, a bug a live benchmark run caught before a unit test did (D-116, T-09) |
| 2026-08-04 | Claude (for Ahmed Saad) | scenarios adds plant/revert_proxy_attribute and cut_proxy_shared_ancestor for T-11: the same fork the common-ancestor scenario plants, read for the opposite purpose (there the sibling is a declared label and the detector must stay silent; here it is a protected attribute and it must raise a candidate). The cut function is the narrowest possible remedy, leaving the classified column and its tag in place so a finding that clears afterwards clears because the ancestry changed, not because the evidence was deleted (D-117) |
