...
change is the "silent" data failure Janus exists to catch: nothing errors,
...
from janus.writeback.labels import ensure_tag
...
SCENARIO_PROPERTY = "janus.scenario"
...
SENSITIVE_TAG_URN = "urn:li:tag:janus.sensitive"
SENSITIVE_TAG_NAME = "janus.sensitive"
SENSITIVE_TAG_DESCRIPTION = (
    "Demo classification: a column whose contents a model must not learn from. "
    "Planted by janus-scenario; stands in for whatever PII or restricted "
...
DEPRECATION_NOTE = (
    "Planted by janus-scenario. Superseded by the v2 feature pipeline; "
...
def revert_schema_drift(conn: DataHubConnection) -> SchemaDriftResult:
    ...
    Re-emits the original :data:`~janus.seed.graph_spec.FEATURE_COLUMNS`, so a
...
def _set_column_lineage(conn: DataHubConnection, mapping: dict[str, list[str]]) -> None:
    ...
    edges, so the next ``janus-seed`` adds its own alongside and the graph
...
def _emit_source_schema(
    ...
    *,
    ...
    what :func:`~janus.seed.seed_ml_graph.seed_warehouse_tables` writes, so
...
PROTECTED_TAG_URN = "urn:li:tag:janus.protected-attribute"
PROTECTED_TAG_NAME = "janus.protected-attribute"
PROTECTED_TAG_DESCRIPTION = (
    "Demo classification: a column holding a protected attribute (race, sex, age, "
    "disability). Planted by janus-scenario; stands in for whatever term an "
...
def _set_model_features(conn: DataHubConnection, *, declared: bool) -> GovernanceResult:
    ...
    if properties is None:
        raise RuntimeError(
            f"{model_urn} has no mlModelProperties; run janus-seed before this scenario."
        )
...
def revert_delinked_model(conn: DataHubConnection) -> GovernanceResult:
    """Re-declare the model's features, the way ``janus link --all`` does."""
...
def _drifted_columns() -> tuple[spec.Column, ...]:
    ...
    Derived from the seeded :data:`~janus.seed.graph_spec.FEATURE_COLUMNS` so
...
def main() -> None:
    """Entry point for the ``janus-scenario`` command."""
    import argparse
    ...
    parser = argparse.ArgumentParser(description="Plant or revert a failure Janus detects.")
    parser.add_argument(
        "--scenario",
    ...
    if args.scenario in (SENSITIVE_SOURCE, DEPRECATED_INPUT):
        ...
        if declared.name == SENSITIVE_SOURCE:
            console.print(
                ...
                f"configured. Set JANUS_SENSITIVE_TAG_URNS={SENSITIVE_TAG_URN} "
