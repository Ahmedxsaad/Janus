...
| 2026-08-05 | Claude (for Ghassen Naouar) | guard_coverage.py lands (D-126, T-15): a pure fold over coverage.py's per-model gaps into one catalog figure, plus the single remedy that would unblock the most checks. It reads nothing, which is what keeps detect/ from importing agent/pipeline to get a sweep. coverage.py's check names become constants and MODEL_CHECKS names the five model-level ones, so a seventh detector cannot open a silent sixth row in a number a platform lead reports upward |
| 2026-08-05 | Claude (for Ghassen Naouar) | blast_radius.py gains upstream_datasets beside downstream_models (D-129, T-18): the same hop-capped, URN-typed read run backwards, because two callers outside detect/ need it and a second copy of a capped lineage read is a second chance to get rule 3 wrong |
| 2026-08-05 | Claude (for Ghassen Naouar) | column_marks.py gains derivation_chains (D-130, T-19): the same walk asked a third question, for the whole derivation rather than the interesting part of it. It reuses split_paths, without which two derivations render as one impossible chain, and drops a path holding only the queried column, which would otherwise print as a complete derivation of a column from itself |
