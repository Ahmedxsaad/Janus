...
from janus.detect.column_marks import WalkResult, marked_ancestor
...
from janus.writeback.properties import FEATURE_TABLE, read_properties
...
_DELINKED = (
    ...
    "Replay it: `janus link --all`, and schedule that after every ingest "
    "(the janus-watch chart ships a CronJob for it).",
)


def was_linked(conn: DataHubConnection, model_urn: str) -> bool:
    """Whether a previous ``janus link`` described this model.
...
def _leakage_gap(
    ...
    if properties is None or not properties.mlFeatures:
        ...
        return gap(
            ...
            "Declare them with `janus link --model ... --features <table> "
    ...
    if not conn.graph.exists(config.label_term_urn):
        return gap(
            ...
            "JANUS_LABEL_TERM_URN at the term your organization already uses.",
        )
    ...
    if unresolved:
        return gap(
            ...
            "declare it with `janus link --model ... --features <table> "
            "--label-column <column>`.",
        )
...
def _cap_reason(
    ...
    if truncated:
        ...
        remedies.append("raise JANUS_LINEAGE_RESULT_CAP")
    if hop_capped:
        reasons.append(
        ...
        remedies.append("raise JANUS_LEAKAGE_MAX_HOPS")
...
def _drift_gap(
    ...
    if properties is None or not properties.trainingJobs:
        return Unevaluated(
            ...
            remedy=(
                ...
                "model's inputs with `janus link`."
            ),
        )
    ...
    if not schema_drift_candidate_resources(conn, model_urn, config):
        return Unevaluated(
            ...
            remedy="Capture one at training time with `janus link`.",
        )
...
def _sensitive_gap(
    ...
    Janus what "sensitive" means in this organization, which is a
    ...
    if not classification_index(conn, config).configured:
        return gap(
            ...
            "Set JANUS_SENSITIVE_TERM_URNS, JANUS_SENSITIVE_TAG_URNS, "
            "JANUS_PROTECTED_ATTRIBUTE_TERM_URNS or "
            "JANUS_PROTECTED_ATTRIBUTE_TAG_URNS to the glossary terms or tags "
...
def _proxy_gap(
    ...
    if not protected_attribute_index(conn, config).configured:
        return gap(
            ...
            "Set JANUS_PROTECTED_ATTRIBUTE_TERM_URNS or "
            "JANUS_PROTECTED_ATTRIBUTE_TAG_URNS to the terms or tags your "
...
def _deprecated_input_gap(
    ...
    if not model_input_datasets(conn, model_urn):
        return Unevaluated(
            ...
            remedy="Declare the model's inputs with `janus link`.",
        )
