...
the column holding the ground truth. Janus reads that path, so a finding is
...
unioned, so a data scientist declaring their label in the UI needs no Janus
configuration at all. The upstream walk, the two aspects it reads, and the
``paths``-not-``urn`` trap that walk turns on all live in
:mod:`janus.detect.column_marks`, because the sensitive-source detector asks
...
from janus.models import LeakageFinding, LeakingFeature, ModelRef
...
def label_index(conn: DataHubConnection, config: ScanConfig) -> ColumnMarkIndex:
    ...
    A :class:`~janus.detect.column_marks.ColumnMarkIndex` over exactly one
...
def leak_path(
    ...
    A thin wrapper over :func:`~janus.detect.column_marks.marked_ancestor`,
