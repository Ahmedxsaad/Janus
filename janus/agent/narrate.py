...
turns a :class:`~janus.models.Finding`, which is already decided, into a few
...
:class:`~janus.llm.LLMConfig`, or None, and :mod:`janus.llm` decides
...
from janus.env import scrub
from janus.llm import LLMConfig, build_chat_model
...
def _freshness_facts(finding: FreshnessFinding) -> str:
    ...
    lines += [
        ...
        "Janus did not query the warehouse.",
    ]
    return "\n".join(lines)
...
def _leakage_facts(finding: LeakageFinding) -> str:
    ...
    return "\n".join(
        [
            ...
            "Janus did not read the data, only the lineage.",
        ]
    )
...
def _drift_facts(finding: SchemaDriftFinding) -> str:
    ...
    lines += [
        ...
        "was trained. Janus compared it against the dataset's current schema; "
...
def _sensitive_facts(finding: SensitiveSourceFinding) -> str:
    ...
    return "\n".join(
        [
            ...
            "DataHub. The derivation is declared in column-level lineage. Janus "
...
def _deprecated_facts(finding: DeprecatedInputFinding) -> str:
    ...
    lines += [
        ...
        f"[{finding.severity}] {'live' if model.is_live else 'not serving'}.",
        "",
        "The deprecation was set by the dataset's own owners. Janus read that "
...
def _proxy_template(finding: ProxyCandidateFinding) -> str:
    ...
    return (
        ...
        f"{candidate.protected_column_name} depends on the data, which Janus has "
...
def _table_level_template(finding: TableLevelRiskFinding) -> str:
    ...
    return (
        ...
        f"{model.name} declares no features, so Janus cannot say which of the "
...
def grounding_facts(finding: Finding, context: str | None = None) -> str:
    ...
    Args:
        ...
        context: Optional organizational context (owners, domain, descriptions)
            ...
            :func:`janus.agent.context_kit.catalog_context`. It joins the
...
def _safe_reason(exc: Exception, llm: LLMConfig) -> str:
    ...
    is clean: remove the key before it reaches a log (janus/env.py scrub).
    """
...
def narrate(finding: Finding, llm: LLMConfig | None, context: str | None = None) -> Narrative:
    ...
    # the same way it reaches an operator's log (janus/argos/handler.py).
