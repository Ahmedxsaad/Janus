...
from janus.agent.context_kit import catalog_context
from janus.agent.narrate import Narrative, incident_description, narrate
from janus.client import DataHubConnection
from janus.config import SCORING_VERSION, ScanConfig
...
from janus.detect.coverage import Unevaluated, coverage_gaps
from janus.detect.degraded import table_level_findings, training_tables
...
from janus.detect.schema_drift import schema_drift_candidate_resources, schema_drift_findings
from janus.detect.trust_score import trust_inputs_from_findings, trust_score
from janus.llm import LLMConfig
from janus.logs import LOG_FIELDS, logfmt
...
from janus.writeback.documents import DocumentWrite, publish_impact_report
...
from janus.writeback.labels import add_tag, ensure_tag, remove_tag
from janus.writeback.process_instance import ScanRun, scan_run
...
from janus.writeback.terms import add_term, ensure_term, remove_term
from janus.writeback.trust_history import TrustEntry, project_history, rendered

#: Description attached to the tag entity the first time it is created.
AT_RISK_TAG_DESCRIPTION = (
    "A Janus scan found this model downstream of a data asset that is failing "
    "its quality or freshness expectations. The model's inputs are not trustworthy "
    "until the upstream asset recovers."
)

#: The term Janus attaches to a feature whose lineage reaches a label column.
LEAKAGE_RISK_TERM_NAME = "leakage-risk"
LEAKAGE_RISK_TERM_DEFINITION = (
    "Janus traced this feature's column-level lineage back to a column declared "
        ...
        if radius is not None:
            ...
            if not radius.models and radius.truncated:
                ...
                warnings.append(
                    ...
                    "JANUS_LINEAGE_RESULT_CAP, or narrow the scan"
                )
                ...
                warnings.append(
                    ...
                    "JANUS_LINEAGE_RESULT_CAP to see the whole blast radius"
                )
...
def _recovery_message(run_id: str) -> str:
    ...
    return f"Recovered by Janus run {run_id}; the finding is no longer present."
...
def _record_recovered_assertion(
    ...
    reads as still breaching the very check Janus wrote to guard it.
...
def _reconcile_stale_findings(
    ...
    *,
    ...
    """Resolve any stale Janus incident and clear the risk it named.
...
def _log_scan(
    ...
    *,
    ...
    message, which stays readable in a terminal tailing ``janus watch``, and
    as structured fields on the record, which :class:`~janus.logs.JsonFormatter`
