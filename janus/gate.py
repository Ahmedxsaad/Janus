...
Janus's tagline is "the missing CI for your ML supply chain", and until this
...
decision here is a function of a :class:`~janus.agent.pipeline.ScanReport`
...
-----------------------------------
Every other Janus entry point writes back: that is the point of it. This one
...
from janus.models import Finding, Severity, severity_rank
...
def github_annotations(verdict: GateVerdict) -> tuple[str, ...]:
    ...
    for violation in verdict.violations:
        ...
        rendered.append(f"::error title=Janus: {title}::{body}")
