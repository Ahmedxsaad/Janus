...
def test_define_properties_emits_one_definition_per_declaration():
    ...
    assert "urn:li:structuredProperty:janus.trust_score" in urns
    assert "urn:li:structuredProperty:janus.trust_band" in urns
...
def test_assign_preserves_properties_this_run_did_not_touch():
    ...
    assert written["urn:li:structuredProperty:janus.trust_score"] == [62.0]
...
def test_a_writer_whose_read_predates_another_write_erases_it():
    ...
    writer per graph (charts/janus-watch), which is a constraint this test
...
def test_assign_replaces_rather_than_appends_an_existing_value():
    stale = StructuredPropertyValueAssignmentClass(
        propertyUrn="urn:li:structuredProperty:janus.trust_score", values=[10.0]
    )
    graph = FakeGraph(
...
def test_read_properties_keys_by_qualified_name():
    assignment = StructuredPropertyValueAssignmentClass(
        propertyUrn="urn:li:structuredProperty:janus.trust_score", values=[62.0]
    )
    graph = FakeGraph(
        aspects={
