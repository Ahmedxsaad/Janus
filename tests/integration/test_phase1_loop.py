...
def scanned(
    ...
    return run_scan(conn, seeded.source_table, config, run_id=run_id, llm=None)
...
def test_the_scan_narrates_without_an_llm_when_none_is_configured(scanned):
    ...
    assert scanned.narrative is not None
    assert scanned.narrative.source is NarrativeSource.TEMPLATE
    assert "Credit Risk v3" in scanned.narrative.assessment


def test_the_incident_lands_on_the_failing_table_and_is_active(
    conn: DataHubConnection, seeded: SeedResult, scanned
):
    assert scanned.incident is not None
    assert scanned.incident.created is True, "the gate must exercise the create path"

    info = conn.graph.get_aspect(scanned.incident.urn, IncidentInfoClass)
...
def test_the_incident_body_states_the_measured_lag(scanned):
    assert scanned.finding is not None
    assert scanned.finding.evidence["lag_hours"].startswith("30.")
...
def test_the_guarding_assertion_exists_on_the_failing_table_and_is_inferred(
    ...
    assert scanned.assertion is not None
    info = conn.graph.get_aspect(scanned.assertion.urn, AssertionInfoClass)
    assert info is not None
...
def test_the_assertion_run_records_the_failure_this_scan_actually_measured(
    ...
    event = _eventually(
        lambda: conn.graph.get_latest_timeseries_value(
            scanned.assertion.urn, AssertionRunEventClass, {}
        ),
...
def test_the_impact_report_is_published_as_a_document_on_the_model(
    ...
    assert len(scanned.documents) == 1
    document = scanned.documents[0]
...
def test_rescanning_reuses_the_incident_rather_than_duplicating_it(
    ...
    second = run_scan(conn, seeded.source_table, config, llm=None)
    assert second.incident is not None
    assert second.incident.created is False, "a second scan duplicated the incident"
    assert second.incident.urn == scanned.incident.urn

    matching = _active_freshness_incidents(conn, seeded.source_table, title)
    assert matching == [scanned.incident.urn], f"expected one active incident, found {matching}"
...
def test_rescanning_reuses_the_assertion_and_the_document(
    ...
    assert second.assertion is not None and scanned.assertion is not None
    assert second.assertion.urn == scanned.assertion.urn
    assert second.assertion.created is False

    assert [d.urn for d in second.documents] == [d.urn for d in scanned.documents]
...
def test_rescanning_does_not_tag_an_already_tagged_model(
    ...
    second = run_scan(conn, seeded.source_table, config, llm=None)
    assert second.tagged_models == (), "the model was tagged twice"
...
def test_once_the_table_refreshes_a_scan_writes_nothing(
    ...
    report = run_scan(conn, seeded.source_table, config, llm=None)
